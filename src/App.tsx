...
const Black: FC = ({ children }) => {
  return <span className="c-black">{children}</span>;
};
...
function App() {
  const jsonStr = JSON.stringify(JSON.parse(json), null, 2);
  const colorized = jsonStr.split("").map((char, index) => {
    switch (char) {
      case " ":
        return <span key={index}>{char}</span>;
      case '"':
        return <Black key={index}>{char}</Black>;
      default: {
